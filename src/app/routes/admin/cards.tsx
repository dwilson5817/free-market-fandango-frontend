...
const Cards = () => {
  ...
  const columns: ColumnDef<components['schemas']['Card-Input']>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <DeleteResource
          onConfirm={() => deleteCard({ params: { path: { card_number: row.original.card_number } } })}
        >
          Confirm you wish to delete card number <strong>{row.original.card_number}</strong> owned by <strong>{row.original.name}</strong>.
        </DeleteResource>
      ),
    },
  ]
